 ...
 * Kept as its own copy so the plugin is self-contained; if the parser gains
...
class LogParser
{
    ...
    private static function parseSingleEntry(string $raw, bool $isLaravel): ?array
    {
        ...
        if ($isLaravel) {
            ...
            // Also matches Craft's own log lines: MonologTarget formats them as
            // "%datetime% [%channel%.%level_name%] [%category%] %message%".
            if (
                ! preg_match(
